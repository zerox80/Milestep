# Milestep Fullstack MVP

Milestep is a Rust fullstack MVP for construction and modernization project planning, built from the supplied mockup.
...
Pop-Location
cargo build --release -p milestep-backend
```

## Session secret (required)

The backend refuses to start unless `MILESTEP_SESSION_SECRET` is set to at least 32 characters. Generate one and put it in `.env`:
...
# Edit .env and set MILESTEP_SESSION_SECRET and POSTGRES_PASSWORD (see above)
...
```yaml
127.0.0.1:${MILESTEP_HTTP_PORT:-80}:80
127.0.0.1:${MILESTEP_POSTGRES_PORT:-5432}:5432
```
...
```powershell
$env:MILESTEP_HTTP_PORT="8080"
docker compose up --build
```

Before enabling it:

- Replace `milestep.example.com`.
...
- In production set `MILESTEP_COOKIE_SECURE=true` for the `app` service (the
  session cookie then uses the `__Host-` prefix).
- In production also set `MILESTEP_PUBLIC_ORIGIN=https://milestep.example.com` so
...
```env
MILESTEP_COOKIE_SECURE=true
MILESTEP_PUBLIC_ORIGIN=https://planner.example.test
```
...
- `MILESTEP_TRUST_PROXY=true` (set automatically in Compose) makes the backend
  ...
  trusts only that subnet (`MILESTEP_TRUSTED_PROXIES=172.30.0.0/24`), so postgres
...
The demo seed is disabled by default. To try the demo, set `MILESTEP_SEED_DEMO=true`
